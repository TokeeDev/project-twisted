...
const EventCard: React.FC<{ event: PrismaEvent }> = ({ event }) => {
  ...
  return (
    <div className="bg-gray-800/50 border border-gray-700 rounded-lg overflow-hidden shadow-lg hover:shadow-twisted-neon/30 transition-shadow duration-300 flex flex-col">
      {event.imageUrl && (
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        {event.description && (
          <p className="text-gray-300 mb-4 text-sm flex-grow line-clamp-3">{event.description}</p>
        )}
...
const EventList: React.FC<EventListProps> = ({ filter }) => {
  ...
  if (events.length === 0) {
    return (
      <div className="text-center text-gray-400 p-8 bg-gray-800/30 rounded-md border border-gray-700">
